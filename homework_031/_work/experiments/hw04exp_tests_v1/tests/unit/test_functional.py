...
class TestRequest:
    ...
    def mock_store():
        class Store:
            ...
            def get(self, key):
                return random.sample(self.interests, 2)
    ...
    @staticmethod
    @pytest.fixture
    def get_response(headers, mock_store, context, get_valid_auth):
        def _get_response(request, use_valid_auth=True):
            body = request
            if use_valid_auth:
                body['token'] = get_valid_auth(request)
            return api.method_handler({"body": body, "headers": headers}, context, mock_store)
    ...
    def test_ok_interests_request(self, get_response, arguments, context):
        ...
        assert all(v for v in response.values()), response
